...
class FrdsCrawler(BaseCrawler):
    def __init__(self, task_config: Dict[str, Any]):
        super().__init__(task_config)
        self.base_url = task_config['site_url'][0]
        setup_logger()
        self.logger = get_logger(name=__name__, site_id="frds")
        self.extract_rules = EXTRACT_RULES.get('frds', [])
        if not self.extract_rules:
            self.logger.warning("未找到frds的数据提取规则")
    ...
    async def _extract_seeding_stats(self, browser: Chromium) -> Dict[str, Any]:
        ...
        try:
            ...
            volumes = await self._extract_seeding_volumes(
                ...
                volume_selector_index= 3  # 体积列选择器（第5列）
            )
