...
def setup_logger():
    ...
    # 日志文件配置
    log_dir = Path(os.getenv('LOG_DIR', settings.BASE_DIR / 'logs'))
    log_file = os.getenv('LOG_FILE', 'crawler_{time}.log')
    error_log_file = os.getenv('ERROR_LOG_FILE', 'error_{time}.log')
    ...
    # 记录日志配置信息
    setup_logger.info("日志配置已加载:")
    setup_logger.info(f"控制台日志级别: {console_log_level}")
    setup_logger.info(f"文件日志级别: {file_log_level}")
    setup_logger.info(f"错误日志级别: {error_log_level}")
    setup_logger.info(f"日志目录: {log_dir}")
    setup_logger.info(f"主日志文件: {log_file}")
    setup_logger.info(f"错误日志文件: {error_log_file}")
